...
class CacheConfigManager:
    ...
    def get_local_cache_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'TIMEOUT': int(self.get_env_value('CACHE_TIMEOUT', '300')),  # 5 minutes default
            'OPTIONS': {
                'MAX_ENTRIES': int(self.get_env_value('CACHE_MAX_ENTRIES', '1000')),
                'CULL_FREQUENCY': int(self.get_env_value('CACHE_CULL_FREQUENCY', '3')),
            }
        }
    ...
    def should_use_redis(self) -> bool:
        ...
        """
        # 默认不使用 Redis，除非明确配置
        use_redis = self.get_env_value('USE_REDIS', 'false').lower() == 'true'
        use_redis_in_debug = self.get_env_value('USE_REDIS_IN_DEBUG', 'false').lower() == 'true'
        
        # 只有在明确配置时才使用 Redis
        if use_redis:
            return True
        elif use_redis_in_debug and self.debug_mode:
            return True
        else:
            return False
    ...
    def print_cache_info(self, custom_specs: Optional[List[CacheBackendSpec]] = None) -> None:
        ...
        print(f"  USE_REDIS: {self.get_env_value('USE_REDIS', 'false')}")
        print(f"  USE_REDIS_IN_DEBUG: {self.get_env_value('USE_REDIS_IN_DEBUG', 'false')}")
        ...
        for spec in specs:
            ...
            if use_redis:
                ...
                print(f"    Backend: Local Memory Cache")
                print(f"    Location: local-dev-cache")
...
def get_django_cache_settings(docker_mode: bool = False, debug_mode: bool = True, 
    ...
    """Generate Django cache settings using the cache manager.
